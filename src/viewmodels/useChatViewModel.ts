...
export const useChatViewModel = (): ChatViewModel => {
  ...
  const handleWebSocketMessage = useCallback((wsMessage: WebSocketMessage) => {
    switch (wsMessage.type) {
      case 'chat':
        ...
        if (taskActionCallbackRef.current && actions?.length > 0) {
  ...
  const sendMessageViaMock = async (content: string) => {
    try {
      ...
      if (taskActionCallbackRef.current && response.actions?.length > 0) {
...
}; API_ENDPOINTS, DEFAULT_MESSAGES } from '../lib/constants';
import { generateId } from '../lib/utils';
